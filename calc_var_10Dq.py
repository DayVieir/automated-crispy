...
def plot_XAS(prefix_XAS, title_plot, suptitle_plot, Dq_lista, desloc_v, ene_i, ene_f, desloc_h, xGaussian):
    ...
    plt.title(title_plot, size=14, fontweight='bold', y=1.018)
    plt.suptitle(suptitle_plot, size=16, fontweight='bold', x=0.52, y=0.96)
    ...
    plt.axis([ene_i,ene_f,-0.018,1.4])
...
q = sg.calculation_file(element='Mn', charge='2+', toCalculate=['Isotropic'], xLorentzian=(0.32, 0.34), hamiltonianState={'Atomic':True,
                    'Crystal Field':True,
...
ten_Dq = [0, 0.3, 0.6, 0.9, 1.2, 1.5, 1.8]
...
titulo='Mn 2+'
subtitulo=''
ene_i=633       #Energia mínima do gráfico
ene_f=655       #Energia máxima do gráfico
shift_v=0.15    #Deslocamento vertical entre gráficos
shift_h=640     #Deslocamento para corrigir a escala de energia do espectro
gauss=0.1
plot_XAS(prefix_XAS, subtitulo, titulo, ten_Dq, shift_v, ene_i, ene_f, shift_h, gauss)
